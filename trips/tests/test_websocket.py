...
class TestWebSocket:
  ...
  async def test_rider_deletes_trip_after_driver_accepts(self,settings):
    settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
    #create rider
    ...
    response_data = response.get('data')
    assert response_data['id'] == trip_id
    assert response_data['status'] == Trip.CANCELLED
    assert response_data['driver']['username'] == driver.username
     # Send a message to the trip group. can be used to notify driver of cancelled trip
    message = {
        'type': 'echo.message',
        'data': 'This is a test message.',
    }
    ...
    await channel_layer.group_send(response_data['id'], message=message)
    #receives message from group using echo_message function in consumers
    response = await channel_layer.receive('test_channel')
    # Rider receives message.
    assert response == message
    await communicator.disconnect()
    
  async def test_driver_can_broadcast_location(self, settings):
    settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
    #create rider
    rider, _ = await create_user(
        'test.user@example.com', 'pAssw0rd', 'rider'
    ...
    #sends messahe to drivers group
    await channel_layer.group_send(trip_id, message=message)
    response = await channel_layer.receive('test_channel')
    # Drivers receives message.
    assert response == message
    await communicator.disconnect()
  async def test_all_drivers_alerted_on_acceptance_of_request(self,settings):
    settings.CHANNEL_LAYERS = TEST_CHANNEL_LAYERS
    # Listen to the 'drivers' group test channel.
    channel_layer = get_channel_layer()
    await channel_layer.group_add(
